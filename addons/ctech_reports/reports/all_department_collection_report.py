    ...
    def data_for_billing_report(self, start_date, end_date, data):
        ...
        for cat in department_list:
            ...
            if bool(account_invoice_line):
                ...
                dict['refund_qty'] = countRefund_qty
                dict['net_qty'] = int(countPaid) - countRefund_qty
                dict['paid_patient'] = len(paid_patient_set)
                dict['free_patient'] = len(free_patient_set)
                final_data.append(dict)

        for list in final_data:
            list['net_amount'] = list['totalPaidAmount'] - list['totalRefundAmount']
            list['total_patient'] = list['countPaid'] + list['countFree']
        return final_data
