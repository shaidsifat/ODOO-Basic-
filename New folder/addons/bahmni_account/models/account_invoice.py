...
class AccountInvoice(models.Model):
    ...
    has_refund = fields.Boolean(string="Has Refund?")
    has_refund_access = fields.Boolean(string="Has Refund Access")
    ...
    def _find_batch(self, product, qty, location, picking):
        ...
        lot_objs = self.env['stock.production.lot'].search(
            [('product_id', '=', product.id), ('life_date', '>=', str(fields.datetime.now()))])
        ...
        if any(lot_objs):
            ...
            self._cr.execute("SELECT SUM(qty) FROM stock_quant WHERE lot_id IN %s and location_id=%s",
                             (lot_ids_for_query, location.id,))
            ...
            if available_qty >= qty:
                ...
                message = (
                              "<b>Auto validation Failed</b> <br/> <b>Reason:</b> There are not enough stock available for <a href=# data-oe-model=product.product data-oe-id=%d>%s</a> product on <a href=# data-oe-model=stock.location data-oe-id=%d>%s</a> Location") % (
                          product.id, product.name, location.id, location.name)
                picking.message_post(body=message)
        else:
            message = (
                          "<b>Auto validation Failed</b> <br/> <b>Reason:</b> There are no Batches/Serial no's available for <a href=# data-oe-model=product.product data-oe-id=%d>%s</a> product") % (
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        '''Invisible Refund button if user has not belong to 'group_has_refund_access' group'''

        result = super(AccountInvoice, self).fields_view_get(view_id, view_type, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
            group_id = self.env.ref("bahmni_sale.group_has_refund_access").id
            if group_id in self.env.user.groups_id.ids:
                if self._context.get('active_model', False) and self._context.get('active_model',
                                                                                  False) == 'sale.order':
                    sale_id = self.env['sale.order'].browse(self._context['active_ids'])
                    invoice_id = sale_id.invoice_ids.filtered(lambda inv_id: inv_id.type == 'out_invoice')
                    invoice_id[0].has_refund_access = True
                elif self._context.get('params', False) and self._context.get('params', False).get('model',
                                                                                                   False) == 'account.invoice':
                    invoice_id = self.browse([self._context['params']['id']])
                    invoice_id.has_refund_access = True
            else:
                if self._context.get('active_model', False) and self._context.get('active_model',
                                                                                  False) == 'sale.order':
                    sale_id = self.env['sale.order'].browse(self._context['active_ids'])
                    invoice_id = sale_id.invoice_ids.filtered(lambda inv_id: inv_id.type == 'out_invoice')
                    invoice_id.has_refund_access = False
                elif self._context.get('params', False) and self._context.get('params', False).get('model',
                                                                                                   False) == 'account.invoice':
                    invoice_id = self.browse([self._context['params']['id']])
                    invoice_id.has_refund_access = False
